...
export const Titlebar = ({ sideBarShow }: { sideBarShow: boolean }) => {
  ...
  }, [menusVisible, closeActiveMenu, setMenusVisible, menuItems, sideBarShow])

  return (
    <div className={`window-titlebar ${wcontext?.platform ? `platform-${wcontext.platform}` : ''}`}>
      {/* Groupe gauche : Icône + Titre + Menu */}
      <div className="window-titlebar-left">
        {/* Icône et titre - masqués si sideBarShow est true */}
        ...
        <>
          ...
          <div className="window-titlebar-title" {...(titleCentered && { 'data-centered': true })}>
            {title}
          </div>
        </>

        {/* Menu toujours visible */}
        {menusVisible && menuItems?.length ? <TitlebarMenu /> : null}
      </div>

      {/* Contrôles Windows toujours visibles à droite */}
